...
- The live transcription setting survives the voice plugin's controller swap. The plugin builds a fresh controller once its speech to text engine loads, and every constructor hardcoded the flag off. (#579)
- The example prompts on an empty chat send instead of doing nothing, in the main window and in the bar's chat pane, and wait behind a loader while the backend connects. (#572, #576)
- The empty chat state scrolls, so the development commands drawer is reachable inside the bar pane. (#577)
