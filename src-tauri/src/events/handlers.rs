...
fn setup_voice_transcription_listeners(app: &AppHandle) {
    ...
    app.listen("voice-transcription:dictation-stopped", move |event| {
        let app_handle = app_handle_for_listener.clone();
        tauri::async_runtime::spawn(async move {
            handle_voice_transcription_dictation_stopped(app_handle, event.payload().to_string()).await;
        });
    });
...
async fn handle_voice_transcription_final_result(app_handle: AppHandle, payload_str: &str) {
    info!("[Event] Received voice-transcription:final-result event: {:?}", payload_str);
...
async fn handle_agent_mode_result(app_handle: AppHandle, extracted_text: Option<String>, payload_str: String) {
    ...
    if let Some(text) = &extracted_text {
        ...
        crate::commands::floating_bar::handle_dictation_finished(&app_handle, Some(query_text)).await;
    }
    ...
    match serde_json::from_str::<serde_json::Value>(&payload_str) {
        Ok(payload_json) => {
            if let Some(text_value) = payload_json.get("text") {
                ...
                error!("[Event] No 'text' field found in final-result payload: {}", payload_str);
            }
        }
        Err(e) => {
...
async fn handle_voice_transcription_dictation_stopped(app_handle: AppHandle, payload: String) {
    ...
    if let Err(e) = app_handle.emit("app-dictation-stopped", payload) {
        error!("[Event] Failed to rebroadcast dictation-stopped event: {}", e);
    }
}
...
async fn handle_dictation_transcription_start(app_handle: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        crate::commands::floating_bar::handle_dictation_mode_change(&app_handle_for_bar, true).await;
    });
    ...
    if let Some(controller_state) = app_handle.try_state::<Arc<Mutex<VoiceController>>>() {
        match tauri_plugin_voice_transcription::commands::start_dictation(
            ...
            Ok(()) => {
                ...
                if let Err(e) = crate::commands::sound::play_voice_start_sound(app_handle.clone(), app_state_for_sound).await {
            ...
            Err(e) => {
                ...
                let app_handle_for_bar = app_handle.clone();
                tauri::async_runtime::spawn(async move {
                    crate::commands::floating_bar::handle_dictation_mode_change(&app_handle_for_bar, false).await;
                });
            }
        }
    } else {
