...
fn handle_agent_mode_shortcut(app: &AppHandle, event: &ShortcutEvent) {
    ...
    if let Err(e) = app.emit(
        crate::constants::events::shortcuts::AGENT_MODE,
        serde_json::json!({
...
fn handle_agent_tap_mode(app: &AppHandle) {
    ...
    if is_dictation_active {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Err(e) = app_handle.emit(crate::constants::events::dictation::STARTED, ()) {
                error!(
...
fn handle_dictation_input_shortcut(app: &AppHandle, event: &ShortcutEvent) {
    ...
    if let Err(e) = app.emit(
        crate::constants::events::shortcuts::DICTATION_INPUT,
        serde_json::json!({
            "state": shortcut_state,
...
pub async fn trigger_shortcut_test_event(
    ...
    let event_name = match shortcut_name.as_str() {
        "agent_mode_toggle" => crate::constants::events::shortcuts::AGENT_MODE,
        "dictation_input" => crate::constants::events::shortcuts::DICTATION_INPUT,
