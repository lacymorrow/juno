...
impl ToolMetadata {
    ...
    fn from_tool_name_patterns(tool_name: &str) -> Self {
        ...
        if let Some(category) = ToolMappingService::get_tool_category(tool_name) {
            let (icon, action_verb, category_name, notification_level, estimated_duration) = match category {
                ToolCategory::AnthropicComputerUse => {
                    match tool_name {
                        ...
                        name if name.contains("key") => ("🔤", "Pressing keys", "Keyboard", "standard", Some("instant")),
                        name if name.contains("scroll") => ("📜", "Scrolling", "Mouse", "minimal", Some("instant")),
                        name if name.contains("drag") => ("🖱️", "Dragging", "Mouse", "minimal", Some("short")),
                        name if name.contains("move") => ("↗️", "Moving cursor", "Mouse", "silent", Some("instant")),
                        _ => ("🖥️", "Screen interaction", "Computer Use", "standard", Some("short"))
                    }
                },
                ToolCategory::Browser => ("🌐", "Browser action", "Browser", "standard", Some("medium")),
                ToolCategory::Desktop => {
                    match tool_name {
                        name if name.contains("click") => ("👆", "Clicking", "Mouse", "minimal", Some("instant")),
                        name if name.contains("type") => ("⌨️", "Typing", "Keyboard", "standard", Some("short")),
                        name if name.contains("application") => ("🖥️", "App control", "Desktop", "standard", Some("short")),
                        name if name.contains("window") => ("🪟", "Window control", "Desktop", "standard", Some("short")),
                        name if name.contains("clipboard") => ("📋", "Clipboard operation", "Desktop", "standard", Some("instant")),
                ...
                ToolCategory::Basic => {
                    match tool_name {
                        ...
                        name if name.contains("command") || name.contains("shell") || name.contains("bash") || name.contains("terminal") => 
                            ("⚡", "Running command", "Command", "detailed", Some("medium")),
                        _ => ("🔧", "Basic operation", "Basic", "standard", Some("short"))
                    }
                },
                ToolCategory::Timer => ("⏰", "Timer action", "Timer", "standard", Some("instant")),
