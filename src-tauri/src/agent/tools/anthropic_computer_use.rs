...
fn extract_line_range(
    ...
    let end_idx = match end_line {
        Some(end) if end == 0 => {
            return Err("Line numbers are 1-indexed, end_line cannot be 0".to_string())
        }
        Some(end) => {
            let end_idx = end;
...
pub async fn execute_computer_tool(
    ...
    match action {
        "screenshot" => {
            ...
            .await
            .map_err(|e| format!("Permission validation failed: {}", e))?;

            let screenshot_path =
                crate::commands::core::capture_screenshot_command(app_handle.clone())
                    .await
        ...
        | "left_click_drag" | "mouse_move" | "left_mouse_down" | "left_mouse_up" => {
            ...
            match action {
                ...
                "left_click_drag" => {
                    ...
                    let (start_x, start_y, end_x, end_y) = if let Some(start_coordinate) =
                        input["start_coordinate"].as_array()
                    {
                        // Old format: start_coordinate + coordinate (end)
                        let coordinate = input["coordinate"].as_array().ok_or_else(|| {
                            "Missing 'coordinate' (end) parameter for drag operation".to_string()
                        })?;

                        let start_x = start_coordinate
                            .get(0)
                            .and_then(|v| v.as_f64())
                            .ok_or_else(|| "Invalid start x coordinate".to_string())?;
                        let start_y = start_coordinate
                            .get(1)
                            .and_then(|v| v.as_f64())
                        ...
                        let end_coordinate =
                            input["end_coordinate"].as_array().ok_or_else(|| {
                                "Missing 'end_coordinate' parameter for drag operation".to_string()
                            })?;

                        let start_x = coordinate
                            .get(0)
                            .and_then(|v| v.as_f64())
                            .ok_or_else(|| "Invalid start x coordinate".to_string())?;
                        let start_y = coordinate
                            .get(1)
                            .and_then(|v| v.as_f64())
                            .ok_or_else(|| "Invalid start y coordinate".to_string())?;
                        let end_x = end_coordinate
                            .get(0)
                            .and_then(|v| v.as_f64())
                            .ok_or_else(|| "Invalid end x coordinate".to_string())?;
                        let end_y = end_coordinate
                            .get(1)
                            .and_then(|v| v.as_f64())
                            .ok_or_else(|| "Invalid end y coordinate".to_string())?;
        ...
        "scroll" => {
            ...
            let scroll_direction = input["scroll_direction"]
                .as_str()
        ...
        "cursor_position" => {
            ...
            let (x, y) =
                crate::commands::mouse::get_cursor_position(app_handle.clone(), state_manager)
                    .await
        ...
        "wait" => {
            ...
            crate::commands::core::wait(seconds, app_handle.clone(), state_manager)
                .await
...
pub async fn execute_bash_tool(
    ...
    let result_json: Value = serde_json::from_str(&result).map_err(|e| {
        // If JSON parsing fails, provide detailed error information
        error!(
            "Failed to parse bash_command result as JSON. Error: {}, Raw result: '{}'",
            e, result
        );
        format!(
...
pub async fn execute_str_replace_tool(
    ...
    match command {
        ...
        "create" => {
            let file_content = input["file_text"]
                .as_str()
...
- Create file: {\"command\": \"create\", \"path\": \"new_file.txt\", \"file_text\": \"content\"}".to_string(),
    ...
    provider
        .register_async_tool(computer_tool, {
            let handle = app_handle.clone();
            move |input: Value| {
                let handle = handle.clone();
                async move { execute_computer_tool(&handle, input).await }
            }
        })
        .await;

    provider
        .register_async_tool(bash_tool, {
            let handle = app_handle.clone();
            move |input: Value| {
                let handle = handle.clone();
    ...
    provider
        .register_async_tool(str_replace_tool, {
            let handle = app_handle.clone();
            move |input: Value| {
                let handle = handle.clone();
