...
use super::types::{
    CloudError, CloudCommand, DeviceResponse, DeviceStatus, DeviceState, SystemInfo, 
...
/// Connection state for the cloud client
...
impl CloudClient {
    ...
    async fn handle_websocket(&self, ws_stream: WebSocketStream<MaybeTlsStream<tokio::net::TcpStream>>) -> Result<(), CloudError> {
        ...
        self.authenticate(&mut ws_sender).await?;
        ...
        let heartbeat_handle = {
            ...
            tokio::spawn(async move {
                let mut heartbeat_timer = time::interval(interval);
                loop {
                    ...
                    if let Ok(message_json) = serde_json::to_string(&heartbeat) {
                        if sender.send(Message::Text(message_json)).await.is_err() {
                            break;
                        }
                    }
                }
    ...
    async fn handle_message(&self, text: String, ws_sender: &mut futures_util::stream::SplitSink<WebSocketStream<MaybeTlsStream<tokio::net::TcpStream>>, Message>) -> Result<(), CloudError> {
        ...
        match message.message_type {
            MessageType::Command => {
                ...
                self.handle_command(command, ws_sender).await?;
            },
            MessageType::Auth => {
    ...
    async fn handle_command(&self, command: CloudCommand, ws_sender: &mut futures_util::stream::SplitSink<WebSocketStream<MaybeTlsStream<tokio::net::TcpStream>>, Message>) -> Result<(), CloudError> {
