...
use serde_json::{Value}; // REMOVED unused json macro
use std::sync::Arc;
use dotenvy::dotenv; // Added for .env loading
use reqwest::{Client, header::{HeaderMap, HeaderValue, ACCEPT, CONTENT_TYPE}}; // Add reqwest client and headers
use std::time::Duration; // Added for sleep
use base64::{engine::general_purpose::STANDARD as BASE64_STANDARD, Engine as _}; // Added for base64 encoding
...
#[tauri::command]
async fn submit_query(query: String, state: tauri::State<'_, AppState>) -> Result<SubmitQueryResult, String> {
    ...
    for iteration in 0..MAX_ITERATIONS {
        ...
        desktop_arc.log("debug", format!("Anthropic Raw Response: {:?}", anthropic_response)); // Log raw response
            ...
            match block.type_.as_str() {
                ...
                "tool_use" => {
                    ...
                    if let (Some(id), Some(name), Some(input)) = (&block.id, &block.name, &block.input) {
                        ...
                        desktop_arc.log("info", format!("Executing tool: {} with input: {:?}", name, input));
                        ...
                        let (content_str, is_error) = match tool_result {
                            Ok(result_value) => {
                                ...
                                desktop_arc.log("info", format!("Tool '{}' success: {}", name, result_str));
                                (result_str, false)
                            }
                            Err(e) => {
                        ...
                        let warn_msg = format!("Warning: Received incomplete tool_use block: {:?}", block);
                        println!("{}", warn_msg);
        ...
        if has_tool_calls {
            ...
            if anthropic_response.stop_reason == "end_turn" || anthropic_response.stop_reason == "stop_sequence" {
                println!("Agent loop finished. Stop reason: {}", anthropic_response.stop_reason);
                desktop_arc.log("info", format!("Agent loop finished. Stop reason: {}", anthropic_response.stop_reason));
                break; // Exit loop, we have the final response
            } else {
                 let warn_msg = format!("Warning: Loop continued without tool calls but stop reason was: {}", anthropic_response.stop_reason);
                 println!("{}", warn_msg);
                 desktop_arc.log("warn", warn_msg);
    ...
    let audio_base64 = match audio_result {
        Ok(base64) => {
            desktop_arc.log("info", "TTS successful, including audio in response.".to_string());
            Some(base64)
        }
        Err(e) => {
            desktop_arc.log("error", format!("TTS failed: {}. Returning response without audio.", e));
...
pub fn run() {
    ...
    let desktop_instance = Desktop::new(false, true).expect("Failed to initialize Desktop Automation Engine");
    let desktop_arc = Arc::new(desktop_instance); // Wrap in Arc for shared ownership

    tauri::Builder::default()
        .manage(AppState { desktop: desktop_arc }) // Add the Desktop instance to Tauri's managed state
