...
impl HeadlessRuntime {
    ...
    pub fn new(app_handle: AppHandle, cli: &Cli) -> Self {
        Self {
            ...
            timeout_duration: Duration::from_secs(cli.get_timeout()),
        }
    }
    ...
    pub async fn execute_command(&self, cli: &Cli) -> Result<HeadlessResult, JunoError> {
        ...
        let result = if cli.daemon {
            ...
            self.execute_batch_mode(Some(batch_file.clone())).await
        } else if cli.interactive {
            self.execute_interactive_mode().await
        } else if let Some(query) = &cli.query {
            self.execute_query(query.clone()).await
        } else if cli.dictation {
            self.execute_dictation(cli.voice_timeout).await
        } else if cli.agent_mode {
            self.execute_agent_mode(cli.voice_timeout).await
        } else if cli.voice_query {
            self.execute_voice_query(cli.voice_timeout).await
        } else if cli.status {
            self.execute_status().await
        } else if let Some(iterations) = cli.iterate {
            self.execute_iterations(iterations, cli.context.clone(), cli.max_depth).await
        } else if cli.self_call {
            self.execute_self_call(cli.context.clone(), cli.max_depth).await
        } else {
        ...
        match result {
            Ok(mut res) => {
                ...
                if self.verbosity >= cli::verbosity::NORMAL && !matches!(self.output_format, OutputFormat::Quiet) {
    ...
    async fn execute_iterations(&self, iterations: u32, context: Option<String>, max_depth: u32) -> Result<HeadlessResult, JunoError> {
        ...
        for i in 0..iterations {
            ...
            if self.app_handle.state::<AppState>().is_cancelled() {
                break;
            }
        }
    ...
    fn output_result(&self, result: &HeadlessResult) {
