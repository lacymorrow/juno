...
pub async fn submit_query(
    ...
    // Use event-driven architecture: Emit user message event
    ...
    // Emit event to trigger event-driven processing
    if let Err(e) = state.emit_event(user_message_event).await {
        ...
        warn!("Event system failed, falling back to direct execution");
        return execute_agent_fallback(trimmed_query.to_string(), state, app_handle).await;
    }
...
/// Fallback execution when event system is not available
async fn execute_agent_fallback(
    query: String,
    state: State<'_, AppState>,
    app_handle: tauri::AppHandle,
) -> Result<(), String> {
    info!("Executing agent via fallback method for query: {}", query);
