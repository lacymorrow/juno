...
use crate::server::types::{
    AppState, OpenApplicationRequest, OpenApplicationResponse,
    OpenApplicationWithElementsResponse,
};
...
> {
    ...
    match desktop.open_application(&request.app_name) {
        Ok(_) => {
            ...
            let mut elements_response = refresh_elements_and_attributes_after_action(
                state.clone(),
            ...
            if elements_response.is_none() {
                log::info!(
                    "elements retrieval failed for {}, retrying after 500ms",
                    request.app_name
                );
                tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
                elements_response = refresh_elements_and_attributes_after_action(
                    state,
                    request.app_name.clone(),
                    500,
                )
                ...
                if elements_response.is_none() {
                    log::warn!(
                        "elements retrieval failed for {} even after retry",
                        request.app_name
                    );
                }
            }
