...
use crate::server::types::ElementStatistics;
use crate::server::types::{
    ElementCache, ElementCacheInfo, ListElementsAndAttributesResponse,
...
pub async fn list_elements_and_attributes_handler(
    ...
    JsonResponse<ListElementsAndAttributesResponse>,
...
> {
    ...
    info!(
        "listing all elements and attributes for app: {}",
        request.app_name
    );
    ...
    info!(
        "excluded {} elements (non-interactable: {}, no text: {})",
    ...
    let element_stats = generate_element_statistics(
        &result_elements,
        excluded_count,
        excluded_non_interactable_count,
        excluded_no_text_count,
    );
    info!(
        "generated statistics: {} different roles found",
        element_stats.top_roles.len()
    );
    ...
    *cache = Some(ElementCache {
        elements: elements.clone(), // Assuming elements is Vec<UIElement>
...
fn generate_element_statistics(
    ...
    for element in elements {
        ...
        if element.get("description").is_some() {
            *property_counts
                .entry("with_description".to_string())
                .or_insert(0) += 1;
        }
        ...
        if element.get("properties").is_some() {
            *property_counts
                .entry("with_properties".to_string())
                .or_insert(0) += 1;
    ...
    let top_limit = 5;
    let top_roles: HashMap<String, u32> = roles.into_iter().take(top_limit).collect();
