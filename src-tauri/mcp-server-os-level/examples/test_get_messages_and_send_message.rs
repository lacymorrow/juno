...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Get all elements
    let all_elements =
        engine.find_elements(&Selector::Text(String::from("")), Some(&messages_app))?;

    info!(
        "found {} total elements in messages app in {:?}",
        all_elements.len(),
        start.elapsed()
    );
    ...
    let sometimes_interactable = ["AXImage", "AXCell", "AXSplitter", "AXRow", "AXStatusItem"];

    // Filter for interactable elements with text
    let interactable_elements: Vec<_> = all_elements
        .iter()
        .enumerate()
        .filter(|(_, element)| {
    ...
    info!(
        "found {} interactable elements with text",
        interactable_elements.len()
    );
    ...
    if let Some((original_index, element)) = message_text_field {
        info!(
            "found message text field at original index: {}",
            original_index
        );
        ...
        match engine.find_element(
            &Selector::Text(String::from("Message")),
            Some(&messages_app),
        ) {
            Ok(element) => {
            ...
            Err(e) => {
                ...
                if interactable_elements.len() > target_index {
                    ...
                    info!(
                        "pressing Return key on element at index {} (original index: {})",
                    ...
                    info!(
                        "not enough elements to access index {}, only have {}",
                        target_index,
                        interactable_elements.len()
                    );
                }
