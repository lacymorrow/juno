...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if active_filters.is_empty() {
        ...
        info!(
            "print filter: only elements that are {}",
            active_filters.join(" AND ")
        );
    }
    ...
    let sometimes_interactable: HashSet<&str> = [
        ...
        "AXOutline",
        "AXOutlineRow",
        "AXHandleElementProxy",
        "AXBrowser",
        "AXColumn",
        "AXGrid",
        "AXWebArea",
        "AXGenericElementProxy",
        "AXValueIndicator",
    ]
    .iter()
    .cloned()
    ...
    for (i, element) in elements.iter().enumerate() {
        ...
        if should_print {
            info!(
                "#{} [{}] {} {}",
                i, role, interactability_marker, element_text
            );
    ...
    info!(
        "  definitely interactable: {} elements ({:.1}%)",
    ...
    info!(
        "  sometimes interactable: {} elements ({:.1}%)",
    ...
    for &idx in &printed_element_indices {
        ...
        writeln!(
            log_file,
            "#{} [{}] {} {}",
            idx, role, interactability_marker, element_text
        )?;
    }
