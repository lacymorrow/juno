...
export const ModelSelector = React.memo(function ModelSelector({
  ...
  const loadModelsForProvider = useCallback(async (providerId: string) => {
    ...
    try {
      ...
      // Ensure models is defined before using it
      if (models) {
      ...
      console.log(
        `ModelSelector: Loaded ${models?.length || 0} models for provider: ${providerId}`,
      );
  ...
  useSafeEventListener(
    EVENTS.SETTINGS_PROVIDER_SETTINGS_CHANGED,
    () => {
    ...
    [settings.activeProvider, loadModelsForProvider],
  );
