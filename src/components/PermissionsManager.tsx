...
export function PermissionsManager({
  ...
  const checkPermissions = useCallback(async (useAutoRedirect = false) => {
    try {
      setIsLoading(true);
      setError(null);

      const result = useAutoRedirect && autoRedirectEnabled
        ? await invoke<PermissionsState>("check_permissions_status_with_auto_redirect", { autoOpenSettings: true })
  ...
  }, [autoRedirectEnabled, onComplete, onRefresh]);
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      if (variant === "splash") {
        ...
        unlistenPermissions = await listen<PermissionsState>("permissions-changed", (event) => {
    ...
    return () => {
      if (variant === "splash") {
        stopMonitoring().catch((err) => {
          console.error("Error stopping permissions monitoring during cleanup:", err);
        });
        unlistenPermissions?.();
      }
    };
  ...
  const getPermissionRequestFunction = (permissionType: string) => {
    switch (permissionType) {
      case "accessibility":
        return () => requestAccessibilityPermissionEnhanced(false);
      case "screen_recording":
  ...
  const renderPermissionCard = (permission: PermissionStatus) => {
    ...
    const onRequestEnhanced = permission.permissionType === "accessibility" && autoRedirectEnabled
      ? () => requestAccessibilityPermissionEnhanced(true)
      : undefined;

    if (variant === "compact") {
      return (
        <div key={permission.permissionType} className="flex items-center justify-between p-2 border rounded">
          <div className="flex items-center gap-2">
            {getPermissionIcon(permission)}
            <span className="text-sm">
              {permission.permissionType.replace("_", " ").replace(/\b\w/g, (l) => l.toUpperCase())}
            </span>
          </div>
          ...
          {!permission.granted && (
            <div className="space-y-3">
              <div className={`p-3 rounded-md border ${
                isRequired ? "bg-red-50 border-red-200" : "bg-yellow-50 border-yellow-200"
              }`}>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {onRequestEnhanced && autoRedirectEnabled && permission.permissionType === "accessibility" && (
                  <Button
                    onClick={onRequestEnhanced}
                    disabled={isRequestingPermission === permission.permissionType}
                    size="sm"
                ...
                {onRequest && (
                  <Button
                    ...
                    size="sm"
                    variant={
                      autoRedirectEnabled && permission.permissionType === "accessibility"
                        ? "outline"
                        : isRequired
                        ? "default"
                        : "secondary"
              ...
              {autoRedirectEnabled && permission.permissionType === "accessibility" && variant === "splash" && (
    ...
    return (
      <div className={`space-y-6 ${className}`}>
        <Alert variant="destructive">
          ...
          <AlertDescription>Error checking permissions: {error}</AlertDescription>
        </Alert>
        <div className="flex gap-2">
          ...
          {autoRedirectEnabled && variant === "splash" && (
            <Button onClick={() => checkPermissions(true)} variant="outline" className="flex-1">
              <Zap className="h-4 w-4 mr-2" />
    ...
    return (
      <div className={`space-y-4 ${className}`}>
        ...
        <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">
              {permissions.allGranted ? "All permissions granted" : "Some permissions missing"}
            </span>
          </div>
        ...
        <div className="flex gap-2 pt-2">
          <Button variant="outline" size="sm" onClick={() => checkPermissions()} disabled={isLoading}>
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      {showHeader && (
        <Card>
          ...
          {!permissions.allGranted && (
            <CardContent>
              <div className="flex flex-wrap gap-2">
                <Button onClick={() => checkPermissions(true)} size="sm" className="bg-green-600 hover:bg-green-700">
                  <RefreshCw className="h-4 w-4 mr-2" />
                  Refresh Status
                </Button>
                {autoRedirectEnabled && (
                  <Button onClick={() => checkPermissions(true)} size="sm" variant="outline">
                    <Zap className="h-4 w-4 mr-2" />
      ...
      {showSkipOption && !permissions.allGranted && onSkip && (
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">You can complete setup later in Settings</p>
        </div>
      )}
    </div>
  );
