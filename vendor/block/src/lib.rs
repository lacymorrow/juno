...
/// Opaque handle for the Objective-C block class symbols exported by libSystem.
///
/// Upstream used `enum Class {}`; an `extern static` of an uninhabited type is
/// rejected by newer rustc (`uninhabited_static`, rust-lang/rust#74840). A
/// zero-sized `#[repr(C)]` struct is the sanctioned opaque-type shape and keeps
/// the same "never construct, only take the address of" semantics.
#[repr(C)]
struct Class {
    _private: [u8; 0],
}
