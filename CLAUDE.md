...
- **Default model: `claude-fable-5-1`** (Anthropic provider). Juno defaults to a model in Anthropic's *current* lineup that Juno can also drive the desktop with; do not downgrade the default for cost, and never default to a model the docs list under "Legacy models (still available)".
- **`claude-opus-5-5` is in the catalog but is deliberately NOT the default.** It is the model Anthropic tells you to start with, but on the Claude API it accepts computer use **only** through the `computer_toolset_20260801` toolset — a `computer_20251124` tool returns a 400 ([what's new in Opus 5.5](https://platform.claude.com/docs/en/models/opus-5-5/whats-new-opus-5-5#computer-20251124-is-not-supported)). Juno now sends the toolset, so Opus 5.5 works, but the toolset ships 17 member tool definitions and costs roughly **2x the input-token overhead on every request** (4,530 vs 2,152 input tokens on an identical call, measured against the live API 2026-09-22). The default stays on a model that takes the cheaper tool.
