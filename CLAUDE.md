...
- **Default model: `claude-opus-5-5`** (Anthropic provider). Juno defaults to a model in Anthropic's *current* lineup that Juno can also drive the desktop with; do not downgrade the default for cost, and never default to a model the docs list under "Legacy models (still available)".
- **`claude-opus-5-5` drives the computer only through `computer_toolset_20260801`** — a `computer_20251124` tool returns a 400 ([what's new in Opus 5.5](https://platform.claude.com/docs/en/models/opus-5-5/whats-new-opus-5-5#computer-20251124-is-not-supported)), confirmed against the live API. Juno now sends that toolset, which is what makes it usable as the default. `claude-fable-5-1` remains in the current lineup and in the picker.
...
- **Computer use runs on two paths, chosen per model from that table.** A `toolset_ga` model sends the toolset: a bare `{"type": "computer_toolset_20260801"}` entry with **no `name`**, no display dimensions and **no beta header** (it is GA on the Claude API), 17 member tools dispatched on the pair (`name`, `toolset_name`) instead of `input.action`, and `toolset_name` echoed back on every `tool_result`. Every other model keeps its single `computer` tool and its beta flag, unchanged — there is a byte-identical regression test for that, and breaking it is a regression, not a refactor. Ask `uses_computer_toolset()`, never `availability`. Do not emit the toolset type without the rest of that contract: a partial implementation returns `invalid_request_error` on **every** request and kills computer use outright.
- The toolset costs roughly **2x the input-token overhead** of the old tool (~4,532 vs ~2,154 measured on otherwise-identical requests) because the 17 member definitions are larger. Prompt caching is the mitigation (LAC-4007); keep the toolset entry covered by a `cache_control` breakpoint.
