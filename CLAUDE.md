...
- **`claude-opus-5-5` is in the catalog but is not the default yet.** It is the model Anthropic tells you to start with, but on the Claude API it accepts computer use **only** through the `computer_toolset_20260801` toolset — a `computer_20251124` tool returns a 400 ([what's new in Opus 5.5](https://platform.claude.com/docs/en/models/opus-5-5/whats-new-opus-5-5#computer-20251124-is-not-supported)). Juno still sends the earlier tools, so Opus 5.5 is declared `ComputerUse::ToolsetOnly` and gets no computer-use tools. **Make it the default in the same change that ports Juno to `computer_toolset_20260801`.**
